...
export default function DeleteSubmissionButton({ submissionId, status, variant = "icon" }: DeleteSubmissionButtonProps) {
    ...
    const trigger = variant === "full" ? (
        <Button
            ...
            className="w-full h-14 gap-3 border-red-500/10 text-red-400 font-black text-xs uppercase tracking-[0.2em] rounded-2xl hover:bg-red-500/5 hover:text-red-600 transition-all italic shadow-inner"
        >
            {loading ? (
            ...
            {isRestricted ? 'Termination Restricted' : 'Terminate Submission Node'}
        </Button>
    ) : (
        <Button
            variant="ghost"
            size="icon"
            disabled={loading || isRestricted}
            className="w-12 h-12 bg-red-500/5 text-red-400 rounded-xl hover:bg-red-500/10 hover:text-red-600 transition-all disabled:opacity-50 shadow-inner"
            ...
            <AlertDialog open={open} onOpenChange={setOpen}>
                ...
                <AlertDialogContent className="rounded-[2.5rem] p-8 bg-white border-red-500/10 shadow-2xl">
                    <AlertDialogHeader className="space-y-4">
                        ...
                        <AlertDialogTitle className="text-2xl font-black text-red-600 tracking-tighter italic">Critical Termination Protocol</AlertDialogTitle>
                        <AlertDialogDescription className="text-xs font-medium text-red-900/40 leading-relaxed uppercase tracking-widest italic">
                    ...
                    <AlertDialogFooter className="pt-6 gap-4">
                        <AlertDialogCancel className="h-14 px-8 rounded-2xl font-black text-[10px] uppercase tracking-widest border-primary/10 text-primary/40 hover:bg-primary/5 italic">
                            Abort Protocol
                        </AlertDialogCancel>
                        <AlertDialogAction
                            onClick={handleDelete}
                            className="h-14 px-8 rounded-2xl bg-red-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-red-700 shadow-xl shadow-red-600/20 italic"
                        >
            ...
            <AlertDialog open={errorOpen} onOpenChange={setErrorOpen}>
                <AlertDialogContent className="rounded-[2.5rem] p-8 bg-white border-primary/5 shadow-2xl">
                    <AlertDialogHeader className="space-y-4">
                        ...
                        <AlertDialogTitle className="text-2xl font-black text-primary tracking-tighter italic">Access Restricted</AlertDialogTitle>
                        <AlertDialogDescription className="text-xs font-medium text-primary/40 leading-relaxed uppercase tracking-widest italic">
                            {errorMessage}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="pt-6">
                        <Button onClick={() => setErrorOpen(false)} className="w-full h-14 font-black text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 italic rounded-2xl">
                            Acknowledge
                        </Button>
                    </AlertDialogFooter>
                </AlertDialogContent>
