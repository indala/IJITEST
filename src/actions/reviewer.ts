...
export async function submitReviewerApplication(formData: FormData) {
    ...
    const validation = schema.safeParse({ fullName, designation, institute, email, application_type });
    ...
    try {
        ...
        const cvUrl = `/uploads/reviewer-apps/${cvName}`;
        const photoUrl = `/uploads/reviewer-apps/${photoName}`;

        // Save to Database
        await pool.execute(
            `INSERT INTO applications (full_name, designation, institute, email, cv_url, photo_url, application_type) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [fullName, designation, institute, email, cvUrl, photoUrl, application_type]
        );
            ...
            html: `
                <div style="font-family: serif; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 40px; border: 1px solid #f0f0f0; border-radius: 20px;">
                    <div style="text-align: center; margin-bottom: 30px;">
                        ...
                        <p style="color: #666; font-size: 14px; text-transform: uppercase; letter-spacing: 0.2em;">${roleName} Board Application</p>
                    </div>
                    <p>Dear ${fullName},</p>
