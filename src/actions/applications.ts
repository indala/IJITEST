...
export async function approveApplication(id: number) {
    try {
        ...
        await pool.execute(
            `INSERT INTO users (email, full_name, designation, institute, role, photo_url, invitation_token, invitation_expires) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [app.email, app.full_name, app.designation, app.institute, role, app.photo_url, invitationToken, expires]
        );
            ...
            html: `
                <div style="font-family: serif; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 40px; border: 1px solid #f0f0f0; border-radius: 20px;">
                    <div style="text-align: center; margin-bottom: 30px;">
                        ...
                        <p style="color: #666; font-size: 14px; text-transform: uppercase; letter-spacing: 0.2em;">Board Enrollment</p>
                    </div>
