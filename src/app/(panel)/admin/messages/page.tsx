...
export default function Messages() {
    ...
    if (loading) return <div className="p-20 text-center font-black text-muted-foreground uppercase tracking-widest text-xs animate-pulse italic">Connecting to Communication Server...</div>;
        ...
        <div className="h-[calc(100vh-140px)] flex flex-col lg:flex-row gap-6 pb-6">
            ...
            <Card className="lg:w-80 flex flex-col border-border/50 shadow-sm overflow-hidden bg-background shrink-0">
                <CardHeader className="p-4 bg-muted/20 border-b border-border/50 space-y-1">
                    ...
                    <CardDescription className="text-[10px] font-medium text-muted-foreground uppercase tracking-tighter">Manage external inquiries</CardDescription>
                </CardHeader>
                <ScrollArea className="flex-1">
                    ...
                    {messages.length === 0 && (
                        <div className="p-12 text-center space-y-3">
                            ...
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest italic">Inbox empty</p>
                        </div>
                    )}
                </ScrollArea>
            </Card>
                ...
                {selectedMessage ? (
                    <>
                            ...
                            <div className="space-y-8 max-w-3xl mx-auto">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] opacity-60">
                                        <AlertCircle className="w-3.5 h-3.5" /> Message Body
                                    </div>
                                ...
                                <div className="flex flex-wrap items-center gap-8 py-4 px-6 bg-muted/20 rounded-xl border border-border/30">
                                    ...
                                    <div className="flex items-center gap-2 text-[9px] font-black text-muted-foreground uppercase tracking-widest">
                                        ...
                                        <span>Status: <Badge variant="secondary" className="h-4 px-1 text-[8px] bg-primary/10 text-primary border-none">{selectedMessage.status}</Badge></span>
                                    </div>
                                </div>
                            </div>
                        </ScrollArea>
                        <div className="p-6 border-t border-border/50 bg-muted/5 flex justify-end">
                            <Button className="h-10 px-6 gap-2 font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-primary/20 rounded-lg">
                                <Send className="w-4 h-4" /> Reply via Journal Mail
                            </Button>
                        </div>
                    </>
                ) : (
                    <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest">Archive Empty</h3>
                            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-widest italic max-w-xs mx-auto">Select a communication record from the left panel to inspect the full content and history.</p>
                        </div>
                    </div>
                )}
            </Card>
