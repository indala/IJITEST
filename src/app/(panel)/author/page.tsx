...
export default async function AuthorDashboard() {
    ...
    let submissions: AuthorDashboardSubmission[] = [];
    let totalViews = 0, totalDownloads = 0, totalCitations = 0;

    try {
        const response = await getAuthorDashboard();
        submissions = (response.data?.submissions || []) as AuthorDashboardSubmission[];

        totalViews = submissions.reduce((acc: number, sub) => acc + (sub.views || 0), 0);
        totalDownloads = submissions.reduce((acc: number, sub) => acc + (sub.downloads || 0), 0);
        totalCitations = submissions.reduce((acc: number, sub) => acc + (sub.citations || 0), 0);
    } catch (error) {
        console.error("Author Dashboard Data Fetch Error:", error);
        return (
